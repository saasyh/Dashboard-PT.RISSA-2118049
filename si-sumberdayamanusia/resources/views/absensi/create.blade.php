...
@section('content_header') 
    <h1 class="m-0 text-dark"><b>Form Data Absensi<b></h1>
@stop
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('id_absensi') is-invalid @enderror" id="id_absensi" placeholder="ID Absensi" name="id_absensi" value="{{old('id_absensi')}}">
                        ...
                        <label for="id_karyawan">Nama Karyawan</label>
                            <select name="id_karyawan" id="id_karyawan" required class="form-control">
                                @foreach ($pegawai as $pgw)
                                    <option value="{{$pgw->id_karyawan}}">{{$pgw->nama}}</option>
                                @endforeach
                           </select>
                    </div>
                    <div class="item form-group">                                
                        <label for="id_pengajuan">Status</label>
                            <select name="id_pengajuan" id="id_pengajuan" required class="form-control">
                                @foreach ($pengajuan as $pgw)
                                    <option value="{{$pgw->id_pengajuan}}">{{$pgw->kode_izin}}</option>
                                @endforeach
                           </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="item form-group">                                
                        ...
                        <select class="input" name="shift" id="shift">
